  ...
  printTokenSlip({
    ...
    age: t.age,
    gender: t.gender,
    bloodPressure: (t as any).bloodPressure,
    phone: t.phone,
    address: t.address,
    doctor: t.doctor,
...
import { Calendar, Clock, User, FileText, BarChart3, Search, Trash2, Undo2, Edit } from 'lucide-react';
...
const TodayTokens = () => {
  ...
  const handleEditClick = (token: Token, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    const withId: any = { ...token, _id: (token as any)._id || (token as any).id };
    try { localStorage.setItem('pendingEditToken', JSON.stringify(withId)); } catch {}
    // Navigate to the Token Generator route so listeners are mounted
    navigate('/tokens');
    // Defer dispatch slightly to ensure component mounts
    setTimeout(() => {
      try { window.dispatchEvent(new CustomEvent('openTokenWithData', { detail: withId })); } catch {}
      try { window.dispatchEvent(new CustomEvent('editToken', { detail: withId })); } catch {}
    }, 50);
  };
  ...
  const openPrescription = (_token: Token) => {
    // Prescription page removed
    return;
  };
  ...
  const exportCsv = () => {
    ...
    const header = Object.keys(rows[0] || { Date: '', Time: '', Token: '', MR: '', Patient: '', Age: '', Gender: '', BP: '', Phone: '', Address: '', Doctor: '', Department: '', Fee: '' });
                ...
                <tbody>
                  {filteredTokens.map((t, idx) => {
                    ...
                    <tr key={(t as any)._id || t.id || idx} className={`border-t hover:bg-gray-50 cursor-pointer ${isReturned ? 'bg-red-50' : ''}`} onClick={() => openPrescription(t)}>
                      ...
                      <td className="px-3 py-2">{(t as any).bloodPressure || ''}</td>
                      ...
                      {isAdmin && (
                        <td className="px-3 py-2 text-right">
                          <div className="inline-flex items-center gap-3">
                            <button title="Edit" onClick={(e) => handleEditClick(t, e)} className="text-indigo-600 hover:text-indigo-800">
